...
from ..schemas.user import UserRequest, User
...
@user.get('', status_code=status.HTTP_200_OK, response_model=List[User])
def get_all(db: Session = Depends(get_db), limit: int = 100,
            current_user: User = Depends(get_current_user)):
    return u.get_users(db, limit)


@user.get('/{user_id}', status_code=status.HTTP_200_OK, response_model=User)
def get_one(user_id: int, db: Session = Depends(get_db),
            current_user: User = Depends(get_current_user)):
    return u.get_one_user(user_id, db)


@user.post('', status_code=status.HTTP_201_CREATED, response_model=User)
